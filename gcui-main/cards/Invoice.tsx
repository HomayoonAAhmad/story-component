  ...
  return (
    <Blocks.Dark>
      <div className="">
          ...
          <div>
            <div className={"flex gap-2 text-xs"}>
              <span className={"text-violet-300 myLtr"}>
                {new Date(invoice.created_at).getHours()} :{" "}
                {new Date(invoice.created_at).getMinutes()}
              </span>
        ...
        <div
          className={
            "border-y border-white border-opacity-10 py-4 text-end flex gap-2 items-center justify-end"
          }
        >
          {
        ...
        <div className={"mt-2 flex justify-between items-center"}>
          <div className={"text-sm"}>
            {status === "expired" && (
              <div className={"text-red-400"}>{Language().invoice_expired}</div>
            )}
            {status === "danger" && (
              <div className={"text-red-400"}>{Language().invoice_danger}</div>
            )}
            {status === "warning" && (
          ...
          <div>
            <Button
              ...
              icon={<span className={"fa fa-chevron-left"}></span>}
            >
              {Language().pay}
            </Button>
          </div>
