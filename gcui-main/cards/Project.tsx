...
export default function Project({ data }: { data: Content }) {
  ...
  let localNader = data.avatar
    ? process.env.NEXT_PUBLIC_UPLOAD_URL + "/" + data.avatar
  ...
  return (
    <Blocks.Gradient>
      <div
        className={
          "flex text-center  sm:text-start flex-wrap sm:flex-nowrap gap-4 items-center"
        }
      >
        <a
          href={"/projects/" + data.slug}
          className={
            "w-full h-12 sm:h-auto sm:w-1/3 block rounded-xl overflow-hidden shadow-lg shadow-slate-900/30"
          }
        >
        ...
        <div className={"w-full"}>
          <a href={"/projects/" + data.slug} className={"block"}>
            <H3 className={"text-indigo-300 mb-2"}>{data.title}</H3>
          </a>
