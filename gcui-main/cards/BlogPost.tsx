...
export default function BlogPost({
  blog,
  compact,
}: Readonly<{ blog: ContentType; compact: boolean }>) {
  if (!blog) {
    return null;
  }
  ...
  return (
    <Blocks.Dark className={"relative"}>
      <div
        className={
          "absolute bottom-0 left-0 w-full h-full bg-bg-cherry v-mask rotate-180 rounded-xl -z-[1]"
        }
      />
        ...
        <div className={"flex items-start justify-between gap-2"}>
          <div>
            <a href={"/blog/" + blog.slug}>
              <H2
                className={
                  "h-16 text-slate-300 text-ellipsis w-full overflow-hidden  line-clamp-2 my-4 md:my-0 font-bold text-lg"
                }
              >
                <span
                  className={"fa fa-link-simple me-2 text-indigo-500"}
                ></span>
            ...
            <Paragraph
              className={
                "text-slate-400 mb-4 text-sm font-light leading-12 h-10 text-ellipsis w-full overflow-hidden  line-clamp-2"
              }
            >
          ...
          {blog.avatar && compact && (
            <div>
              <div
                className={
                  "w-24 h-24 rounded-xl overflow-hidden shadow-lg shadow-violet-900/30"
                }
              >
      ...
      <div
        className={
          "flex gap-2 border-t border-white border-opacity-10 pt-4 mt-4"
        }
      >
        ...
        <div className={"flex flex-col"}>
          ...
          <Paragraph className={"text-slate-500 text-xs"}>{date}</Paragraph>
        </div>
