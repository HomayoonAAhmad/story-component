...
const Auth_confirmSms = async (mobile, code) => {
  ...
  formData.append(
    "key",
    `${rdd.engineName}-${rdd.engineVersion}-${rdd.fullBrowserVersion}`
  );
  ...
  formData.append(
    "type",
    rdd.isMobile ? "mobile" : rdd.isTablet ? "tablet" : "desktop"
  );

  let response = false;
  let token = "";
  await AuthModel.loginConfirmSms(
    formData,
    (data) => {
      token = data.token;
      response = true;
...
export { Auth_check, Auth_logout, Auth_sendSms, Auth_confirmSms };
