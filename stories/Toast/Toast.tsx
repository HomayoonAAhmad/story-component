...
const ToastElement = ({ message }) => {
  ...
  return (
    ...
    >
      <div className={"w-12"}>
        <span
          className={`far fa-${localMessage.icon} ${
            localMessage.type === ColorTypes.danger
              ? "text-red-500"
      ...
      <div className={""}>
        ...
        <div className={"text-slate-200 "}>{localMessage.message}</div>
      </div>
    </div>
  );
};
const Toast = () => {
  ...
  let message: ToastProps = useSyncExternalStore(
  ...
  return (
    <div
      className={`fixed bottom-12 left-0 w-full md:w-96 md:left-2 md:bottom-2 z-50`}
    >
